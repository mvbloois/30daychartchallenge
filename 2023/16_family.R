...
europe <- giscoR::gisco_get_countries(year = "2020",
                                      epsg = "4326",
                                      resolution = "10")
...
df <- read_delim("./2023/data/ilc_peps01n_tabular.tsv.gz") %>%
  clean_names() %>%
  separate(
    col = freq_unit_age_sex_geo_time_period,
...
ggplot() +
  ...
  scale_fill_gradientn(
    name = "% children at risk of poverty or social exclusion",
    colours = cols2,
    breaks = brk,
    labels = brk,
  ...
  labs(
    ...
    subtitle = str_wrap(
      "Children under 16 years who are at risk of poverty or severely materially or socially deprived or living in households with very low work intensity.",
      75
    ),
    caption = "\nData: EuroStat"
  ) +
